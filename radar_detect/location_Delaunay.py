...
class location_Delaunay(object):
    ...
    def _cal_pos_edge(self, pts: np.ndarray, pt) -> np.ndarray:
        ...
        if not self._check(pts):
            ...
            # 德劳内投影点对应世界坐标点
            h_1 = self.w_points[np.argmax(np.apply_along_axis(all, 1, self.cam_points == pts[0]))]
            h_2 = self.w_points[np.argmax(np.apply_along_axis(all, 1, self.cam_points == pts[1]))]
            # 定比分点计算待定位点坐标
